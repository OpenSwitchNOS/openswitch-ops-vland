    ...
    "VLAN Identifier\n")
{
    ...
    if (vlan_found)
    {
        ...
        OVSREC_PORT_FOR_EACH(port_row, idl)
        {
            ...
            {
                if (vlan_id == ops_port_get_trunks(port_row, i, idl))
                {
                    vlan_found = 1;
                    ...
                    for (i = n = 0; i < port_row->n_vlan_trunks; i++)
                    {
                        if (vlan_id != ops_port_get_trunks(port_row, i, idl))
                        {
                            trunks[n++] = ops_port_get_trunks(port_row, i, idl);
                        }
                    }
            ...
            if (port_row->vlan_tag != NULL && ops_port_get_tag(port_row, idl) == vlan_id) {
                vlan_found = 1;
            }
    ...
    "VLAN identifier\n")
{
    ...
    if (vlan_id != 0 && ops_port_get_tag(vlan_port_row, idl) != vlan_id)
    {
    ...
    "VLAN identifier range. [2, 2-10 or 2,3,4 or 2,3-10]\n")
{
    ...
    while (list != NULL)
    {
        ...
        {
            if (vlan_id == ops_port_get_trunks(vlan_port_row, i, idl))
            {
        ...
        for (i = 0; i < vlan_port_row->n_vlan_trunks; i++)
        {
            trunks[i] = ops_port_get_trunks(vlan_port_row, i, idl);
        }
    ...
    for (i = 0; i < vlan_port_row->n_vlan_trunks; i++)
    {
        if (vlan_id == ops_port_get_trunks(vlan_port_row, i, idl))
        {
            ...
            for (i = n = 0; i < vlan_port_row->n_vlan_trunks; i++)
            {
                if (vlan_id != ops_port_get_trunks(vlan_port_row, i, idl))
                {
                    trunks[n++] = ops_port_get_trunks(vlan_port_row, i, idl);
                }
            }
    ...
    for (idx = 0; idx < count; idx++)
    {
        ...
        if (port_row != NULL)
        {
            OVSREC_PORT_FOR_EACH(port_row, idl)
            {
                ...
                for (i = 0; i < port_row->n_vlan_trunks; i++)
                {
                    if (vlan_row->id == ops_port_get_trunks(port_row, i, idl))
                    {
                        if (port_row->vlan_tag != NULL && ops_port_get_tag(port_row, idl) == vlan_row->id)
                        {
                            print_tag = 1;
                        }
                ...
                if (print_tag == 0 && port_row->vlan_tag != NULL && ops_port_get_tag(port_row, idl) == vlan_row->id)
                {
                    port_nodes[n++] = (struct ovsrec_port *)port_row;
                }
            }
