    ...
    "VLAN identifier\n")
{
    ...
    /* Checking  wether vlan is used as trunk native mode. */
    OVSREC_PORT_FOR_EACH(port_row, idl)
    {
    ...
    "VLAN identifier\n")
{
    ...
    /* Checking wether vlan is used as trunk allowed mode. */
    OVSREC_PORT_FOR_EACH(port_row, idl)
    {
       for (i = 0; i < port_row->n_trunks; i++) {
          if (vlan_id == port_row->trunks[i]) {
              vty_out(vty,"Failed to set !! VLAN%d used as trunk allowd %s",vlan_id, VTY_NEWLINE);
              cli_do_config_abort(status_txn);
              return CMD_SUCCESS;
         }
       }
