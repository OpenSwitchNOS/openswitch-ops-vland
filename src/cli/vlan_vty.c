    ...
    "VLAN identifier range. Example: [2, 2-10 or 2,3,4 or 2,3-10]\n")
{
    ...
    while (list != NULL)
    {
        ...
        if (!check_iface_in_bridge(ifname))
        {
            vty_out(vty, "Failed to set allowed trunk VLAN. Disable routing on the interface %s.%s", ifname, VTY_NEWLINE);
            cli_do_config_abort(status_txn);
            return CMD_SUCCESS;
        }
        ...
        if (NULL == vlan_row)
        {
            vty_out(vty, "VLAN %d not found for interface %s, aborting all the VLAN's ", vlan_id, ifname);
        ...
        for (i = 0; i < vlan_port_row->n_trunks; i++)
        {
            ...
            {
                vty_out(vty, "The VLAN %d is already allowed on the interface %s.%s", vlan_id, ifname, VTY_NEWLINE);
                status = cli_do_config_finish(status_txn);
                if (status == TXN_SUCCESS || status == TXN_UNCHANGED)
                {
                    return CMD_SUCCESS;
