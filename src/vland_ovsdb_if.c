...
vland_ovsdb_init(const char *db_path)
{
    ...
    ovsdb_idl_omit_alert(idl, &ovsrec_bridge_col_name);
...
static int
create_default_vlan(struct ovsdb_idl_txn *status_txn)
{
    ...
    static char vlan_name[16] = { 0 };
    snprintf(vlan, 5, "%d", vlan_id);
    snprintf(vlan_name, 9, "%s%d", "DEFAULT_VLAN_", vlan_id);
