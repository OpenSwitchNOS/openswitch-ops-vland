...
def vlan_trunk_allowed_positive(dut, hs1, hs2, step):
    ...
    assert ping['transmitted'] == ping['received'] == 1, "host2 should be" \
        "reachable from host1 as trunk is allowed for vlan 2"
    ping = hs2.libs.ping.ping(1, "3.3.3.3")
    assert ping['transmitted'] == ping['received'] == 1, "host1 should be" \
...
def test_vlan_trunk_allowed(topology, step):
    ...
    step("##### 3. Test to no vlan trunk interface #########################")
