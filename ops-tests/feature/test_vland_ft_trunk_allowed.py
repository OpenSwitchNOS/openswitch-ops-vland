...
def vlan_trunk_allowed_positive(dut, hs1, hs2, step):
    ...
    # if the number of transmitted packets is greater than or equal to the
    # number of received packets and both should be atleast 5 to pass the test
    ping = hs1.libs.ping.ping(10, "2.2.2.3")
    assert ping['transmitted'] >= ping['received'] >= 5, "host2 should be" \
        " reachable from host1 as trunk is allowed for vlan 2"
    ping = hs2.libs.ping.ping(10, "3.3.3.3")
    assert ping['transmitted'] >= ping['received'] >= 5, "host1 should be" \
