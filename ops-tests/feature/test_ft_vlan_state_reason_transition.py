...
@mark.test_id(15303)
def test_ft_vlan_state_reason_transition(topology):
    ...
    print("Step 3- Verify vlans were configured")
    assert findvlan(ops1, "2")
    assert findvlan(ops1, "3")
    assert findvlan(ops1, "4")

    print("Step 5- Verify vlan reason before enabling it")
    ...
    print("Step 10- Verify vlan status")
    assert vlanstatus(ops1, '2', 'status', 'up')
    assert vlanstatus(ops1, '3', 'status', 'down')
    assert vlanstatus(ops1, '4', 'status', 'down')

    print("Step 11- Verify vlan reason")
    assert vlanstatus(ops1, '2', 'reason', 'ok')
    assert vlanstatus(ops1, '3', 'reason', 'admin_down')
    assert vlanstatus(ops1, '4', 'reason', 'admin_down')

    print("Step 12- Disable vlan")
    with ops1.libs.vtysh.ConfigVlan('2') as ctx:
        ctx.shutdown()

    print("Step 13- Verify vlan status down")
    assert vlanstatus(ops1, '2', 'status', 'down')
    assert vlanstatus(ops1, '3', 'status', 'down')
    assert vlanstatus(ops1, '4', 'status', 'down')

    print("Step 14- Verify vlan reason")
    assert vlanstatus(ops1, '2', 'reason', 'admin_down')
    assert vlanstatus(ops1, '3', 'reason', 'admin_down')
    assert vlanstatus(ops1, '4', 'reason', 'admin_down')

    print("Step 15- Verify ping between hosts")
    ping = hs1.libs.ping.ping(3, '192.168.30.11')
    assert ping['transmitted'] == ping['errors'] == 3
