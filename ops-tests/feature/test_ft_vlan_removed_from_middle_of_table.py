...
def test_ft_vlan_removed_from_middle_of_table(topology):
    ...
    print("Step 4- Add ports to vlans")
    ...
    print("Step 5- Enable interfaces")
    with ops1.libs.vtysh.ConfigInterface('7') as ctx:
        ctx.no_shutdown()
    with ops1.libs.vtysh.ConfigInterface('8') as ctx:
        ctx.no_shutdown()
    ...
    print("Step 8- Delete middle vlan from table")
    with ops1.libs.vtysh.Configure() as ctx:
        ctx.no_vlan(3)

    print("Step 9- Verify vlan was deleted")
    assert findvlan(ops1, "3") == 0

    print("Step 10- Re-assign port to other vlan")
    with ops1.libs.vtysh.ConfigInterface('8') as ctx:
        ctx.no_routing()
        ctx.vlan_access(2)
    sleep(2)

    print("Step 11- Verify ports re-assign correctly")
    assert findport_vlan(ops1, '2', '8') == 1

    print("Step 12- Send traffic")
    raw = hs1("nmap -sP 10.1.1.1-254")
    result = re.findall('\(\d*\s\w*\s\w*\)', raw)
    hostnumber = re.findall('\d?', result[0])
