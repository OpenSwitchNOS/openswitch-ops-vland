...
@mark.test_id(15300)
def test_ft_vlan_id_validation(topology):
    ...
    print("Step 1- Invalid vlan not configured")
    ...
    print("Step 2- Verify no previous vlans are configured")
    countv = getvlan(ops1)
    if len(countv) == 0:
    ...
    print("Step 3- Add vlan")
    with ops1.libs.vtysh.ConfigVlan('2') as ctx:
        ctx.no_shutdown()

    print("Step 4- Verify vlan was configured")
    assert findvlan(ops1, "2") == 1

    print("Step 5- Try to add repeated vlan")
    with ops1.libs.vtysh.ConfigVlan('2') as ctx:
        ctx.no_shutdown()

    print("Step 6- Validate vlan repeated not configured")
    assert findvlan(ops1, "2") == 1
