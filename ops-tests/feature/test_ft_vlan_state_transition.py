...
def test_ft_vlan_state_transition(topology):  # noqa
    ...
    print("Step 3- Verify vlan state")
    assert vlanstatus(ops1, '2', 'status', 'down')

    print("Step 4- Add port to vlan")
    with ops1.libs.vtysh.ConfigInterface('7') as ctx:
        ctx.no_routing()
        ctx.vlan_access(2)

    print("Step 5- Verify ports assign correctly")
    assert findport_vlan(ops1, '2', '7')

    print("Step 6- Enable vlan")
    with ops1.libs.vtysh.ConfigVlan('2') as ctx:
        ctx.no_shutdown()

    print("Step 7- Verify vlan state")
